...
export default class Game {
  constructor() {//
    ...
    const port = 3000;
    this.client_socket = new WebSocket(`ws://localhost:${port}`);
    this.client_socket.addEventListener('open', () => {
      // Causes the server to print "Hello"
     console.log('Sending message to mf server');
      this.client_socket.send('{"type":"new_guest"}');
  })
  ...
  process_message(message) {
    ...
    switch(message_from_server.type) {
      ...
      case 'turn_result' :
        ...
        if(message_from_server.ss_success === true) {
      ...
      case 'guest_list':
        // This case is to be removed when lobby's ws and persistent data works on the server
        if(message_from_server.length > 0) {
          let index = 0;
          for(index ; index <= message_from_server.length ; index++ ) {
